...
;(function( $, document, window, undefined ) {
	...
	var updateElementState = function( element, settings ){
		if( isElementVisible( element, settings ) ){
			...
			if( tSupport ){
				...
				element.css({ visibility: 'visible' }).stop().animate({ opacity: 1 }, settings.speed, settings.easing, function(){
	...
	*/
	...
	var isElementVisible = function( element, settings ){
		...
		var elBottom = ( adjustedOffsetTop + element.outerHeight() ) - ( element.outerHeight() * settings.threshold );
		var elTop = adjustedOffsetTop + ( element.outerHeight() * settings.threshold );
